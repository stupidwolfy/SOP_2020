package kmitl.sop.week6.LabTest;
...
public class App {
 ...
 public List<Car>car_pool = new ArrayList<Car>();
 ...
 public ResponseEntity<List<Car>> addcars(@RequestBody List<Car> cars){
  cars.stream().forEach(c -> car_pool.add(c));
 ...
 public ResponseEntity<List<Car>> patch(@RequestBody List<Car> cars, @PathVariable ("index") int index){
  cars.stream().forEach(car -> car_pool.set(index-1, car));
